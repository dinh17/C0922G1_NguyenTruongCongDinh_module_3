...
--      so_luong_dich_vu_di_kem (được tính dựa trên việc sum so_luong ở dich_vu_di_kem), tien_dat_coc của tất cả các dịch vụ đã từng được khách hàng đặt vào 
...
where 
(
quarter(hd.ngay_lam_hop_dong) in (4) 
and 
year(hd.ngay_lam_hop_dong)='2020'
) 
and not 
(
quarter(hd.ngay_lam_hop_dong) in(1,2,3)
 and 
...
having count(hd.ma_nhan_vien) <=3 and (hd.ngay_
